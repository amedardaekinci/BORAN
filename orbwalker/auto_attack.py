#!/usr/bin/env python3
"""
Auto Attack — Range içine giren düşmana otomatik saldır.

Mantık:
1. Hero ve düşman pozisyonlarını oku
2. 2D mesafe hesapla (X-Z düzlemi)
3. Düşman attack range içindeyse → A + sol tık (attack-move-click)
...
def main():
    print("=" * 60)
    print("  BORAN Auto Attack")
    print("  Range içine giren düşmana otomatik saldır")
    ...
    try:
        while True:
            ...
            # Range içinde düşman var mı?
            in_range = closest_dist <= local_range

            if tick % 20 == 0:
                status = f"✓ {closest_name} @ {closest_dist:.0f}" if in_range else f"  nearest={closest_dist:.0f}"
                print(f"[{tick:5d}] HP:{local_hp:.0f} Range:{local_range:.0f} {status}")

            # Saldır!
            if in_range and now > attack_cooldown:
                print(f"  >>> ATTACK! {closest_name} dist={closest_dist:.0f} < range={local_range:.0f}")
                attack_click()
